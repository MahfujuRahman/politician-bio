...
export const store = defineStore("volunteer_page", {
  ...
  getters: {
    ...
    is_form_valid: (state) => {
      return (
        state.form_data.first_name.trim() !== "" &&
        state.form_data.last_name.trim() !== "" &&
        state.form_data.email.trim() !== "" &&
        state.form_data.phone_number.trim() !== "" &&
        Object.keys(state.form_data.selected_prerequisites).length > 0
      );
    },
  },
  ...
  actions: {
    ...
    async fetch_prerequisite() {
      if (await this._isCacheValid("prerequisite")) {
        this.prerequisite = this._cache["prerequisite"].data;
        return;
      }
      try {
        const res = await axios.get("volunteer-pre-requisites", {
          params: {
            get_all: 1,
            limit: 1000,
          },
        });
        this.prerequisite = res.data.data;
        await this._setCache("prerequisite", res.data.data);
      } catch (e) {
        this.error = e;
      }
    },
    ...
    validate_form() {
      ...
      if (Object.keys(this.form_data.selected_prerequisites).length === 0) {
        this.errors.selected_prerequisites = [
          "Please select at least one option from the prerequisites",
        ];
        is_valid = false;
      }
    ...
    toggle_prerequisite_option(prereq_id, option) {
      if (!this.form_data.selected_prerequisites[prereq_id]) {
        this.form_data.selected_prerequisites[prereq_id] = [];
      }
      const index =
        this.form_data.selected_prerequisites[prereq_id].indexOf(option);
      if (index > -1) {
        this.form_data.selected_prerequisites[prereq_id].splice(index, 1);
      } else {
        this.form_data.selected_prerequisites[prereq_id].push(option);
      }
      // Clear error when user makes a selection
      if (
        this.errors.selected_prerequisites &&
        Object.keys(this.form_data.selected_prerequisites).length > 0
      ) {
        delete this.errors.selected_prerequisites;
      }
    },
